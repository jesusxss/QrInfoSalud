...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if (!function_exists('password_hash')) {
            $errors[] = "La función password_hash no está disponible en este hosting.";
        } else {
            ...
            if (!$stmt) {
                ...
                $stmt->bind_param("sss", $nombre_usuario, $correo, $contrasena_cifrada);
                if (!$stmt->execute()) {
...
<body>
  ...
  <script>
    // Mostrar modal de términos al cargar la página
    document.addEventListener('DOMContentLoaded', function() {
      // Solo mostrar si no hay errores
      <?php if (empty($errors)): ?>
    ...
    document.getElementById('formRegistro').addEventListener('submit', function(e) {
      ...
      if (errores.length > 0) {
        ...
        if (errores.some(error => error.includes('términos'))) {
          mostrarTerminos();
        }
      }
    });
